<template>
    <main class="app-content">
      <div class="app-title row">
             <div class="col-md-8">
                <h1><i class="bi bi-collection"></i> Ciudad: </h1> 
             </div>
                <div class="col-md-4">
                    <select class="form-select" @change="onChangeCiudad($event)" v-model="ciudad">
                        <option :value=0>Seleccione...</option>
                        <option v-for="ciudad in arrayCiudad" :value="ciudad.id" v-text="ciudad.desc"></option>
                    </select>
                </div>
      </div>
      <div class="row justify-content-center">
        <div class="col-md-4 mb-3">
            <div class="card shadow-lg border-secondary text-white rounded-lg mb-3">
                <div class="card-header bg-secondary"><strong>Unidad Administrativa</strong></div>
                <div class="card-body">
                    <div class="row">
                         <div class="col-md-12">
                ...
                <div class="card-footer text-end">
                    <button class="btn btn-primary" type="button" @click="abrirModalUnidad('registrar')"><i class="bi bi-plus-square"></i> Nuevo</button>
                    <button class="btn btn-warning" type="button" @click="abrirModalUnidad('actualizar')"><i class="bi bi-pencil-square"></i>Modificar</button>
                </div>
            </div>
            <div class="card shadow-lg border-secondary text-white rounded-lg mb-3">
                <div class="card-header bg-secondary">Establecimiento</div>
                <div class="card-body">
                    <div class="row">
                         <div class="col-md-12">
                            <label for="">Seleccione:</label>
                            <select name="" id="" class="form-select" @change="onChangeEdif($event)" v-model="edif">
                                <option :value=0>Seleccione...</option>
                                <option v-for="edif in arrayEdif" :value="edif.id" v-text="edif.sigla + ' - ' + edif.descripcion"></option>
                            </select>
                        </div>
                    </div>
                </div>
                <div class="card-footer text-end">
                    <button class="btn btn-primary mt-2" type="button" @click="abrirModalEstab('registrar')"><i class="bi bi-plus-square"></i> Nuevo</button>
                    <button class="btn btn-warning mt-2" type="button" @click="abrirModalEstab('actualizar')"><i class="bi bi-pencil-square"></i>Modificar</button>
                </div>
            </div>
  ...
  methods: {
    async obtenerCiudades() {
        try {
            ...
            this.arrayCiudad = response.data;
        } catch (error) {
            console.error('Error al obtener las ciudades', error);
        }
    },
    onChangeCiudad(e){
        this.ciudad=e.target.value;
        this.listarUnidad(this.ciudad);
        this.idunidad=0;
        this.edif=0;
        this.limpiaroficina();
    },
    ...
    onChangeUnidad(e){
        ...
        this.listarEdif(this.idunidad);
        this.edif=0;
        this.limpiaroficina();
    },
    listarEdif(id){
         let me=this;
            var url= '/establecimiento/'+id;
            axios.get(url).then( (response) =>{
            if(response.data){
                var respuesta= response.data;
                me.arrayEdif = respuesta.establecimientos;
            }
        })
        .catch( (error)=> {
            console.log(error);
    ...
    onChangeEdif(event){
        this.edif=(event.target.value);
        this.limpiaroficina();
    },
    buscarOficina(){
        let me=this;
        var url= '/oficinas/buscarOficina?filtro=' + me.codofic + '&edif=' + me.edif;
        axios.get(url).then((response)=>{
    ...
    listarOficina(page, buscar,criterio){
        let me=this;
        var url= '/oficinas/?page=' + page + '&buscar='+ buscar + '&criterio='+ criterio + '&edif=' + this.edif;
        axios.get(url).then( (response) =>{
            var respuesta= response.data;
