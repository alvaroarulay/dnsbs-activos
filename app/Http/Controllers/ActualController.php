...
{   
    ...
    public function store(Request $request)
    {
        try{
            ...
            if($cantidad == 1){
                    ...
                    $actual->entidad = '0015';
                    $actual->codigo = $request->codigoparcial.'-'.$request->correlativo + 1;
                    ...
                    for($i=0;$i<$cantidad;$i++) {
                        ...
                        $actual->unidad = $request->unidad;
                        $actual->entidad = '0015';
                        $actual->codigo = $request->codigoparcial.'-'.$request->correlativo + ($i+1);
                        $actual->codcont = $request->codcont;
                        $actual->codaux = $request->codaux;
                        $actual->vidautil = $vidautil->vidautil;
                        $actual->descripcion = $request->descripcion;
    ...
    public function generarcodigo(Request $request){
        $unidad=Unidadadmin::where('unidad','=',$request->unidad)->first();
        $nrociudad=Ciudad::where('id','=',$unidad->idciudad)->first();
        ...
        $sigofic=Establecimiento::where('id','=',$oficina->idestablecimiento)->first();
        $correlativo = Actual::where('codcont','=',$gcont->codcont)->count();

        $codigo=$gcont->codigo.'-'.$nrociudad->codigo.$sigofic->sigla.'-'.$oficina->codofic;
