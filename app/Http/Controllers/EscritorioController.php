...
class EscritorioController extends Controller
{
    ...
    public function grafica1()
    {
        $idrol = \Auth::user()->idrol;
        if($idrol==1){
            $estados = Actual::join('estado','estado.id','=','actual.codestado')->select('estado.nomestado',DB::raw('count(*) as valor'))->groupBy('estado.nomestado')->get();
        }
        else{
                $estados = Actual::join('estado','estado.id','=','actual.codestado')->select('estado.nomestado',DB::raw('count(*) as valor'))->where('unidad','=',\Auth::user()->unidad)->groupBy('estado.nomestado')->get();
            }
